...
class Constraint:
    ...
    def value(
        ...
        if self.ctype == ConstrainedVariableType.INPUT_AND_STATE:
            ...
            x = input
        return np.round_(
            np.atleast_1d(np.squeeze(self.sym_func(np.array(x, ndmin=1)))), decimals=self.rounding
        )
    ...
    def check_tolerance_shape(self):
        """Check if the tolerance shape matches the number of constraints."""
        if self.tolerance is None:
            return
...
class ConstraintList:
    ...
    def symbolic(self, state_models: bool = True, input_models: bool = True) -> list[Callable]:
        """Return all the symbolic models of the constraints."""
    ...
    @staticmethod
    def from_specs(
        state_space: Space, input_space: Space, constraint_specs: list[dict]
    ) -> ConstraintList:
        ...
        for constraint in constraint_specs:
            ...
            constraint_list.append(c_class(state_space, input_space, **kwargs))
